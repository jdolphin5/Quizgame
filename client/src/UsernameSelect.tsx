...
const UsernameSelect: React.FC<UsernameSelectProps> = ({ usernameSelected, setUsernameSelected, userState, setUserState }) => {
    ...
    useEffect(() => {
        if (generateUsername) {
            axios.get(`http://localhost:3000/api/generate-username`)
            .then((response: any) => {
    ...
    const setUsername = (event : React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
        ...
        if (inputUsername.length === 0) {
            ...
            setHelperText("Username can only contain numbers, letters or underscores");
                    ...
                    <div className="usernameGrid">
                        <div className="username-row-2">
                            <form autoComplete="off" onSubmit={handleSubmit}>
                                <TextField value={userInput} id="standard-basic" label="Enter Username" variant="standard" helperText={helperText} error={usernameError} onChange={setUsername} />
                                <Button style={{maxHeight: '100px', minHeight: '42px'}} variant="contained" onClick={() => {setGenerateUsername(!generateUsername)}}>Generate Username</Button>
                                <br />
                                <br />
                                <br />
